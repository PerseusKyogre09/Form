  ...
  async function handleImageUpload(e: Event) {
    ...
    if (file) {
      try {
        ...
        formData.append("path", `animation-blocks/${fileName}`);
...
<div
  class="border border-gray-200 rounded-xl p-6 bg-white hover:border-gray-300 hover:shadow-md transition-all duration-200"
>
  <div class="flex items-start gap-4 mb-4">
    ...
    <div class="flex-1 space-y-3">
      <div class="flex flex-wrap items-center gap-3">
        <span
          class="inline-flex items-center justify-center text-xs font-semibold tracking-wide px-3 py-1 rounded-full bg-purple-100 text-purple-700"
        >
          Animation Block
        </span>
        <button
      ...
      <div class="space-y-2">
        <label
          class="text-xs font-semibold text-gray-500 uppercase tracking-wide"
          >Animation Image (optional)</label
        >
        <input
          type="file"
          accept="image/*"
  ...
  <div class="space-y-2">
    ...
    <div
      class={`animation-preview animation-${animation.animationType}`}
      style="background: {animation.backgroundColor ||
        'transparent'}; border: 1px solid {animation.backgroundColor ===
      'transparent'
        ? 'rgba(229, 231, 235, 1)'
    ...
    >
      ...
      {:else}
        <div class="text-center space-y-2">
          <span
            class="text-sm font-semibold {animation.backgroundColor ===
            'transparent'
              ? 'text-gray-600'
              : 'text-white'}">{animation.title || "Animation"}</span
          >
          {#if animation.enableAutoAdvance}
            <p
              class="text-xs {animation.backgroundColor === 'transparent'
                ? 'text-gray-400'
