...
>
  {#each $notifications as notification (notification.id)}
    <div
      ...
      class="flex items-center gap-3 px-5 py-4 {getBackgroundColor(
        notification.type,
      )} text-white rounded-lg shadow-2xl pointer-events-auto backdrop-blur-sm border border-white/10 dark:border-gray-800/10"
    >
      <i class="fas {getIcon(notification.type)} text-lg animate-pulse"></i>
      <span class="text-sm font-medium">{notification.message}</span>
      <button
        on:click={() => handleRemove(notification.id)}
        class="ml-2 hover:bg-white/20 dark:hover:bg-black/20 p-1 rounded transition-all"
